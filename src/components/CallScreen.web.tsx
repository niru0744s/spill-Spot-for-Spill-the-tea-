 ...
 * CallScreen.web.tsx
 * ------------------
 * Web-specific call overlay component. Renders full-screen dialogs for
 * dialing, ringing, active voice calls, and active video calls on browser.
 * Uses agoraService (Web RTC Engine wrapper) under the hood.
 */
...
export function CallScreen() {
  ...
  useEffect(() => {
    ...
    const startAgora = async () => {
      ...
      if (!hasGranted) {
        console.warn('[CallScreenWeb] Hardware permissions not granted, ending call.');
      ...
      try {
        ...
        agoraService.registerEventHandler({
          onJoinChannelSuccess: (cName) => {
            console.log('[AgoraWeb] Joined channel successfully:', cName);
            if (isMounted) setIsJoining(false);
          },
          onUserJoined: (remoteUserUid) => {
            console.log('[AgoraWeb] Remote user joined:', remoteUserUid);
            if (isMounted) setRemoteUid(remoteUserUid);
          },
          onUserOffline: (remoteUserUid) => {
            console.log('[AgoraWeb] Remote user offline:', remoteUserUid);
            if (callId) endCall(callId);
          },
          onError: (err, msg) => {
            console.error('[AgoraWeb] Error code:', err, 'Message:', msg);
          },
        });

        // D. Join Channel
        await agoraService.joinChannel(agoraToken, channelName, 0, type || 'voice', {
          isMuted,
          isSpeaker,
        });

      } catch (error) {
        console.error('[CallScreenWeb] Agora setup error:', error);
        if (callId) endCall(callId);
      }
    };
  ...
  // Handle playing video feeds on screen transitions
  useEffect(() => {
    if (status === 'accepted' && type === 'video') {
  ...
  useEffect(() => {
    if (status === 'accepted' && type === 'video' && remoteUid) {
      const playRemote = async () => {
        await new Promise((resolve) => setTimeout(resolve, 150));
        agoraService.playRemoteVideo(remoteUid, 'remote-video-container');
      };
      playRemote();
    }
  }, [status, type, remoteUid]);

  const cleanupAgora = async () => {
    try {
      await agoraService.leaveChannel();
    } catch (err) {
      console.warn('[CallScreenWeb] Error releasing Agora Web client:', err);
    }
    setRemoteUid(null);
    setIsJoining(false);
  };
      ...
      <View style={styles.container}>
        ...
        {isVideo ? (
          <View style={styles.videoGrid}>
            ...
            {/* Local Preview Container (PiP) */}
            <View style={styles.localPipWindow} id="local-video-container" />
          </View>
        ) : (
        ...
        <View style={styles.controlBarContainer}>
          <View style={styles.controlBar}>
            ...
            {/* Speakerphone (Toggle audio routing - Web standard default) */}
            <TouchableOpacity
